...
class P extends Component {
  ...
  componentDidMount(){
    if(this.props.page == 0){
      this.props.action.classNoficeList({page_no:1,page_size:10});
    }
  }
  ...
  onMore(){
    this.props.action.classNoficeList({page_no:this.props.page+1,page_size:10});
  }
      ...
      <ScrollView style={{paddingVertical:10}} refreshControl={
        ...
        {this.props.list.map(o=>(
          <TouchableOpacity key={o.id} style={{
              ...
              flexDirection:'row'}} onPress={()=>{this.props.action.selectMessageClass({id:o.id, dataset:'account'});Actions.messageClass();}}>
...
export default connect(
  state=>{
    let type = _find(state.messageList.typeList, {id:'2'});
    let list = _filter(state.messageList.list, {type:'2'});
    list = list.map(o=>{
      //let s = _find(state.messageList.stateList, {id:o.stateId});
      return {
        ...o,
        content: `${o.title}-${o.class_name}`,
    ...
    return {
      list,
      loading:state.messageList.classLoading|| false,
      page:state.messageList.classPage||0,
      over:state.messageList.classOver||false,
    }
  },
  dispatch=>({
    action: bindActionCreators({
      selectMessageClass: action.selectMessageClass,
      classNoficeList: action.classNoficeList,
    }, dispatch)})
)(P);
